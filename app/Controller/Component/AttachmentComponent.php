...
class AttachmentComponent extends Component {
	...
	function initialize(&$controller) {
		...
		$this -> config = array_merge(array('default_col' => $prefix), /* default column prefix is lowercase, singular model name */
	...
	function upload_FS(&$data) {
		$column_prefix = $this -> config['default_col'];
		$error = 0;
		$tmpuploaddir = WWW_ROOT . 'attachments' . DS . 'tmp';
		// /tmp/ folder (should delete image after upload)
		...
		unset($data[$column_prefix]);
		/* delete $_FILES indirection */
